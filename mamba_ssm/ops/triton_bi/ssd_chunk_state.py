...
def _chunk_cumsum_fwd_kernel(
    ...
    dt_ptr, A_ptr, dt_bias_ptr, dt_out_ptr, dA_cumsum_ptr, dA_rev_cumsum_ptr,
    # Matrix dimension
    ...
    dA_rev_cumsum_ptr += pid_b * stride_dA_cs_batch + pid_c * stride_dA_cs_chunk
    ...
    dA_rev_cs_ptrs = dA_rev_cumsum_ptr + (offs_h[:, None] * stride_dA_cs_head + offs_c[None, :] * stride_dA_cs_csize)
    ...
    dA_rev_cs = tl.cumsum(dA, axis=1, reverse=True)
    tl.store(dA_cs_ptrs, dA_cs, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size))
    tl.store(dA_rev_cs_ptrs, dA_rev_cs, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size))
...
def _chunk_state_fwd_kernel(
    # Pointers to matrices
    x_ptr, b_ptr, states_ptr, states_b_ptr, dt_ptr, dA_cumsum_ptr, dA_rev_cumsum_ptr, seq_idx_ptr,
    # Matrix dimensions
    ...
    stride_dA_cs_batch, stride_dA_cs_chunk, stride_dA_cs_head, stride_dA_cs_csize,
    stride_dA_rev_cs_batch, stride_dA_rev_cs_chunk, stride_dA_rev_cs_head, stride_rev_dA_cs_csize,
    ...
    dA_rev_cumsum_ptrs = dA_rev_cumsum_ptr + offs_k * stride_rev_dA_cs_csize
    ...
    acc_rev = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.float32)
    for k in range(0, chunk_size_limit, BLOCK_SIZE_K):
        ...
        dA_rev_cs_k = tl.load(dA_rev_cumsum_ptrs, mask=offs_k < chunk_size_limit - k, other=0.0).to(tl.float32)
        if HAS_SEQ_IDX:
        ...
        if not HAS_SEQ_IDX:
            # [(A0:B) - (A0:1)], (A0:B) - (A0:2), (A0:B) - (A0:3), ... , (A0:B) - (A0:B)
            # [(A1:B), (A2:B), (A3:B), ... , (AB:B)]
            scale = tl.exp((dA_cs_last - dA_cs_k)) * dt_k
            # [(A0:B) - (A0:B)], (A0:B) - (A1:B), (A0:B) - (A2:B), ... , (A0:B) - (AB-1:B)
            # [(A0:0), (A0:1), (A0:2), ... , (A0:B-1)]
            scale_rev = tl.exp((dA_rev_cs_first - dA_rev_cs_k)) * dt_k
        else:
            scale = tl.where(seq_idx_k == seq_idx_last, tl.exp((dA_cs_last - dA_cs_k)) * dt_k, 0.0)
            scale_rev = tl.where(seq_idx_k == seq_idx_last, tl.exp((dA_rev_cs_first - dA_rev_cs_k)) * dt_k, 0.0)
        b_rev = b * scale_rev[:, None]
        b_rev = b_rev.to(x_ptr.dtype.element_ty)
        b *= scale[:, None]
        b = b.to(x_ptr.dtype.element_ty)
        acc += tl.dot(x, b)
        acc_rev += tl.dot(x, b_rev)

        x_ptrs += BLOCK_SIZE_K * stride_x_seqlen
        b_ptrs += BLOCK_SIZE_K * stride_b_seqlen
        dt_ptrs += BLOCK_SIZE_K * stride_dt_csize
        dA_cumsum_ptrs += BLOCK_SIZE_K * stride_dA_cs_csize
        dA_rev_cumsum_ptrs += BLOCK_SIZE_K * stride_rev_dA_cs_csize
        if HAS_SEQ_IDX:
            seq_idx_ptrs += BLOCK_SIZE_K * stride_seq_idx_seqlen
    states = acc.to(states_ptr.dtype.element_ty)
    states_b = acc_rev.to(states_b_ptr.dtype.element_ty)
...
def _chunk_state_bwd_db_kernel(
    # Pointers to matrices
    x_ptr, dstates_ptr, dstates_b_ptr, b_ptr, dt_ptr, dA_cumsum_ptr, dA_rev_cumsum_ptr, seq_idx_ptr,
    db_ptr, ddA_cumsum_ptr, ddA_rev_cumsum_ptr,
    # Matrix dimensions
    ...
    dstates_ptr += pid_b * stride_dstates_batch + pid_c * stride_dstates_chunk + (pid_g * (nheads // ngroups) + pid_s * nheads_per_program) * stride_dstates_head
    ...
    if HAS_DDA_CS:
        ...
        ddA_rev_cumsum_ptr += pid_b * stride_ddA_rev_cs_batch + pid_c * stride_ddA_rev_cs_chunk + (pid_g * (nheads // ngroups) + pid_s * nheads_per_program) * stride_ddA_rev_cs_head
    if HAS_SEQ_IDX:
        seq_idx_ptr += pid_b * stride_seq_idx_batch + pid_c * chunk_size * stride_seq_idx_seqlen
    ...
    dstates_ptrs = dstates_ptr + (offs_n[None, :] * stride_dstates_dstate + offs_k[:, None] * stride_dstates_hdim)
    ...
    for h in range(nheads_iter):
        ...
        db = tl.dot(x, dstates)
        db_rev = tl.dot(x, dstates_b)
        dA_cs_last = tl.load(dA_cumsum_ptr + (chunk_size - 1) * stride_dA_cs_csize).to(tl.float32)
        # It's the same as dA_cs_last, so no need to load it again
        # dA_rev_cs_first = tl.load(dA_rev_cumsum_ptr).to(tl.float32)

        dA_cs_m = tl.load(dA_cumsum_ptrs, mask=offs_m < chunk_size, other=0.0).to(tl.float32)
        dA_rev_cs_m = tl.load(dA_rev_cumsum_ptrs, mask=offs_m < chunk_size, other=0.0).to(tl.float32)
        dt_m = tl.load(dt_ptrs, mask=offs_m < chunk_size, other=0.0).to(tl.float32)
        if not HAS_SEQ_IDX:
        ...
        db *= (scale * dt_m)[:, None]
        db_rev *= (scale_b * dt_m)[:, None]
        if HAS_DDA_CS:
            # This is the gradient wrt (dA_cs_last - dA_cs_m), i.e. the exclusive reverse cumsum
            ddA_cs = tl.sum(db * b, axis=1)
            # The same goes for the reverse cumsum
            ddA_rev_cs = tl.sum(db_rev * b, axis=1)
            tl.atomic_add(ddA_cumsum_ptrs + stride_ddA_cs_csize, ddA_cs, mask=offs_m < chunk_size - 1)
            tl.atomic_add(ddA_rev_cumsum_ptrs - stride_ddA_rev_cs_csize, ddA_rev_cs, mask=(offs_m < chunk_size) & (offs_m > 0))
        acc += db + db_rev
        x_ptrs += stride_x_head
        dstates_ptrs += stride_dstates_head
...
def _chunk_cumsum_fwd(dt, A, chunk_size, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    dA_rev_cumsum = torch.empty(batch, nheads, nchunks, chunk_size, device=dt.device, dtype=torch.float32)
    grid_chunk_cs = lambda META: (batch, nchunks, triton.cdiv(nheads, META['BLOCK_SIZE_H']))
    with torch.cuda.device(dt.device.index):
        _chunk_cumsum_fwd_kernel[grid_chunk_cs](
            dt, A, dt_bias, dt_out, dA_cumsum, dA_rev_cumsum,
            batch, seqlen, nheads, chunk_size,
    ...
    return dA_cumsum, dA_rev_cumsum, dt_out


#TODO # Update the backward pass
...
def _chunk_state_fwd(B, x, dt, dA_cumsum, dA_rev_cumsum, seq_idx=None, states=None, states_b=None, states_in_fp32=True):
    ...
    if states is not None:
        ...
        assert states_b.shape == states.shape
    else:
    ...
    with torch.cuda.device(x.device.index):
        _chunk_state_fwd_kernel[grid](
            x, B, states, states_b, dt, dA_cumsum, dA_rev_cumsum, seq_idx,
...
def _chunk_state_bwd_db(x, dt, dA_cumsum, dA_rev_cumsum, dstates, dstates_b, seq_idx=None, B=None, ngroups=1):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    ...
    if B is not None:
        ...
        ddA_rev_cumsum = torch.empty(batch, nheads, nchunks, chunk_size, device=x.device, dtype=torch.float32)
        ddA_cumsum_strides = (ddA_cumsum.stride(0), ddA_cumsum.stride(2), ddA_cumsum.stride(1), ddA_cumsum.stride(3))
        ddA_rev_cumsum_strides = (ddA_rev_cumsum.stride(0), ddA_rev_cumsum.stride(2), ddA_rev_cumsum.stride(1), ddA_rev_cumsum.stride(3))
    else:
        B_strides = (0, 0, 0, 0)
        ddA_cumsum = None
        ddA_rev_cumsum = None
        ddA_cumsum_strides = (0, 0, 0, 0)
        ddA_rev_cumsum_strides = (0, 0, 0, 0)
    ...
    with torch.cuda.device(x.device.index):
        _chunk_state_bwd_db_kernel[grid_db](
            x, dstates, dstates_b, B, dt, dA_cumsum, dA_rev_cumsum, seq_idx, dB, ddA_cumsum, ddA_rev_cumsum,
    ...
    if ddA_rev_cumsum is not None:
        ddA_rev_cumsum = ddA_rev_cumsum.flip([-1])
        torch.cumsum(ddA_rev_cumsum, dim=-1, out=ddA_rev_cumsum)
        ddA_rev_cumsum = ddA_rev_cumsum.flip([-1])
        # ddA_rev_cumsum = rev_cumsum(ddA_rev_cumsum)
    return dB if B is None else (dB, ddA_cumsum, ddA_rev_cumsum)
...
@triton.jit
def _chunk_state_bwd_db_no_atmoic_kernel(
    # Pointers to matrices
    x_ptr, dstates_ptr, dstates_b_ptr, b_ptr, dt_ptr, dA_cumsum_ptr, dA_rev_cumsum_ptr, seq_idx_ptr,
    db_ptr, ddA_cumsum_ptr, ddA_rev_cumsum_ptr,
    # Matrix dimensions
    ...
    for h in range(nheads_iter):
        ...
        if HAS_DDA_CS:
            ...
            #     tl.device_print("", ddA_cs)
            # tl.atomic_add(ddA_cumsum_ptrs + stride_ddA_cs_csize, ddA_cs, mask=offs_m < chunk_size - 1)
            tl.store(ddA_cumsum_ptrs + stride_ddA_cs_csize, ddA_cs, mask=offs_m < chunk_size - 1)
            # tl.atomic_add(ddA_rev_cumsum_ptrs - stride_ddA_rev_cs_csize, ddA_rev_cs, mask=(offs_m < chunk_size) & (offs_m > 0))
            tl.store(ddA_rev_cumsum_ptrs - stride_ddA_rev_cs_csize, ddA_rev_cs, mask=(offs_m < chunk_size) & (offs_m > 0))
        acc += db + db_rev
        x_ptrs += stride_x_head
        dstates_ptrs += stride_dstates_head
        dstates_b_ptrs += stride_dstates_b_head
...
def _chunk_state_bwd_db_no_atmoic(x, dt, dA_cumsum, dA_rev_cumsum, dstates, dstates_b, seq_idx=None, B=None, ngroups=1):
    batch, seqlen, nheads, headdim = x.shape
    _, _, nchunks, chunk_size = dt.shape
    dstate = dstates.shape[-1]
    assert dt.shape == (batch, nheads, nchunks, chunk_size)
    ...
    if B is not None:
        ...
        ddA_cumsum = torch.empty(batch, nheads, nchunks, chunk_size, accum_axis_size, device=x.device, dtype=torch.float32)
        ddA_rev_cumsum = torch.empty(batch, nheads, nchunks, chunk_size, accum_axis_size, device=x.device, dtype=torch.float32)
    ...
    grid_db = lambda META: (triton.cdiv(chunk_size, META['BLOCK_SIZE_M']) * triton.cdiv(dstate, META['BLOCK_SIZE_N']),
                        batch * nchunks, nsplits * ngroups)
    with torch.cuda.device(x.device.index):
        _chunk_state_bwd_db_no_atmoic_kernel[grid_db](
            x, dstates, dstates_b, B, dt, dA_cumsum, dA_rev_cumsum, seq_idx, dB, ddA_cumsum, ddA_rev_cumsum,
            chunk_size, dstate, headdim,
    ...
    if ddA_cumsum is not None:
        ...
        # torch.cumsum(ddA_cumsum[..., 1:], dim=-1, out=ddA_cumsum[..., 1:])
        # But it's easier to just do the cumsum for all elements, the result will be the same.
        ddA_cumsum = torch.sum(ddA_cumsum, dim=-1)
        torch.cumsum(ddA_cumsum, dim=-1, out=ddA_cumsum)
    if ddA_rev_cumsum is not None:
        ddA_rev_cumsum = torch.sum(ddA_rev_cumsum, dim=-1)
        ddA_rev_cumsum = ddA_rev_cumsum.flip([-1])
        torch.cumsum(ddA_rev_cumsum, dim=-1, out=ddA_rev_cumsum)
        ddA_rev_cumsum = ddA_rev_cumsum.flip([-1])
        # ddA_rev_cumsum = rev_cumsum(ddA_rev_cumsum)
    # ddA_cumsum, ddA_rev_cumsum = sum_cumsum(ddA_cumsum, ddA_rev_cumsum)
    return dB if B is None else (dB, ddA_cumsum, ddA_rev_cumsum)
