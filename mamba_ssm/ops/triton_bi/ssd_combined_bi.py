...
from mamba_ssm.ops.triton_bi.ssd_bmm import _bmm_chunk_fwd, _bmm_chunk_bwd
from mamba_ssm.ops.triton_bi.ssd_chunk_state import _chunk_cumsum_fwd, _chunk_cumsum_bwd
from mamba_ssm.ops.triton_bi.ssd_chunk_state import _chunk_state_fwd, _chunk_state_bwd_db
from mamba_ssm.ops.triton_bi.ssd_chunk_state import _chunk_state_bwd_db_no_atmoic as _chunk_state_bwd_db_old
from mamba_ssm.ops.triton_bi.ssd_chunk_state import _chunk_state_bwd_ddAcs_stable
from mamba_ssm.ops.triton_bi.ssd_chunk_state import chunk_state, chunk_state_ref
from mamba_ssm.ops.triton_bi.ssd_state_passing import _state_passing_fwd, _state_passing_bwd
from mamba_ssm.ops.triton_bi.ssd_state_passing import state_passing, state_passing_ref
from mamba_ssm.ops.triton_bi.ssd_chunk_scan import _chunk_scan_fwd, _chunk_scan_bwd_dz, _chunk_scan_bwd_dstates
from mamba_ssm.ops.triton_bi.ssd_chunk_scan import _chunk_scan_bwd_dcb, _chunk_scan_bwd_dC
from mamba_ssm.ops.triton_bi.ssd_chunk_scan import _chunk_scan_bwd_dC_no_atomic as _chunk_scan_bwd_dC_old
from mamba_ssm.ops.triton_bi.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_stable
from mamba_ssm.ops.triton_bi.ssd_chunk_scan import chunk_scan, chunk_scan_ref
from mamba_ssm.ops.triton_bi.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_prev
from mamba_ssm.ops.triton_bi.layernorm_gated import rmsnorm_fn, _layer_norm_fwd, _layer_norm_bwd
from mamba_ssm.ops.triton_bi.k_activations import _swiglu_fwd, _swiglu_bwd

from mamba_ssm.ops.triton.ssd_combined import mamba_chunk_scan_combined

TRITON_22 = version.parse(triton.__version__) >= version.parse('2.2.0')
import time
from mamba_ssm.modules.ssd_minimal_bi import ssd_minimal_discrete_bi
...
@triton.autotune(
    configs=[
        ...
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 32}, num_stages=3, num_warps=4, pre_hook=init_to_zero(["ddt_ptr"])),
    ],
...
def _chunk_scan_chunk_state_bwd_dx_kernel(
        ...
        x_ptr, cb_ptr, dout_ptr, dt_ptr, dA_cumsum_ptr, dA_rev_cumsum_ptr, seq_idx_ptr, D_ptr,
        b_ptr, dstates_ptr, dstates_b_ptr,
        ...
        stride_dstates_b_batch, stride_dstates_b_chunk, stride_dstates_b_head, stride_dstates_b_hdim,
    ...
    dA_cs_m = tl.load(dA_cumsum_ptr + offs_m * stride_dA_cs_csize, mask=offs_m < chunk_size_limit, other=0.0).to(
        tl.float32)
    dA_rev_cs_m = tl.load(dA_rev_cumsum_ptr + offs_m * stride_dA_rev_cs_csize, mask=offs_m < chunk_size_limit,
                          other=0.0).to(tl.float32)

    dA_cs_last = tl.load(dA_cumsum_ptr + (chunk_size - 1) * stride_dA_cs_csize).to(tl.float32)
    # dA_rev_cs_first = tl.load(dA_rev_cumsum_ptr).to(tl.float32)
    if not HAS_SEQ_IDX:
        scale = tl.exp(dA_cs_last - dA_cs_m)
        scale_b = tl.exp(dA_cs_last - dA_rev_cs_m)
    else:
        seq_idx_m = tl.load(seq_idx_ptr + offs_m * stride_seq_idx_seqlen, mask=offs_m < chunk_size_limit, other=-1)
        seq_idx_last = tl.load(seq_idx_ptr + (chunk_size_limit - 1) * stride_seq_idx_seqlen)
        scale = tl.where(seq_idx_m == seq_idx_last, tl.exp(dA_cs_last - dA_cs_m), 0.0)
        scale_b = tl.where(seq_idx_m == seq_idx_last, tl.exp(dA_cs_last - dA_rev_cs_m), 0.0)
    ...
    dstates_b_ptrs = dstates_b_ptr + (
                offs_n[None, :] * stride_dstates_b_hdim + offs_dstate[:, None] * stride_dstates_b_dstate)
    if IS_TRITON_22 and BLOCK_SIZE_DSTATE <= 128:
        ...
        for k in range(0, dstate, BLOCK_SIZE_K):
            ...
            dstates_b = tl.load(dstates_b_ptrs, mask=(offs_dstate[:, None] < dstate - k) & (offs_n[None, :] < hdim),
                                other=0.0)
            dstates_b = dstates_b.to(b_ptr.dtype.element_ty)
            acc_b += tl.dot(b, dstates_b)
            b_ptrs += BLOCK_SIZE_K * stride_b_dstate
            dstates_ptrs += BLOCK_SIZE_K * stride_dstates_dstate
            dstates_b_ptrs += BLOCK_SIZE_K * stride_dstates_b_dstate
        acc *= scale[:, None]
        acc += acc_b * scale_b[:, None]
    ...
    for k in range(K_MIN, K_MAX, BLOCK_SIZE_K):
        ...
        dA_rev_cs_k = tl.load(dA_rev_cumsum_ptrs, mask=offs_k < K_MAX - k, other=0.0).to(tl.float32)

        diag_cumsum = dA_cs_k[None, :] - dA_cs_m[:, None]
        diag_rev_cumsum = dA_rev_cs_k[None, :] - dA_rev_cs_m[:, None]
        diag_cumsum = tl.where((offs_m[:, None] <= k + offs_k[None, :]), diag_cumsum, 0)
        diag_rev_cumsum = tl.where((offs_m[:, None] >= k + offs_k[None, :]), diag_rev_cumsum, 0)
        diag_cumsum = tl.exp(diag_cumsum + diag_rev_cumsum)
        diag_cumsum = tl.where(offs_m[:, None] == k + offs_k[None, :], 2.0, diag_cumsum)
        # if ((tl.program_id(axis=0) == 0) and (tl.program_id(axis=1) == 0)) and ((tl.program_id(axis=2) == 0) and k == 32):
        # if ((pid_m == 1) and (tl.program_id(axis=1) == 0)) and ((tl.program_id(axis=2) == 0) and k == 32):
        #     tl.device_print("k", k, diag_cumsum)

        # if k == 32:
        #     tl.device_assert(tl.sum(diag_cumsum - diag_rev_cumsum) == 0)
        cb *= diag_cumsum
...
def _chunk_scan_chunk_state_bwd_dx(x, dt, dA_cumsum, dA_rev_cumsum, B, CB, dout, dstates, dstates_b, D=None,
                                   seq_idx=None, dx=None):
    ...
    assert dstates_b.shape == dstates.shape
    ...
    with torch.cuda.device(x.device.index):
        _chunk_scan_chunk_state_bwd_dx_kernel[grid_dx](
            x, CB, dout, dt, dA_cumsum, dA_rev_cumsum, seq_idx, D, B, dstates, dstates_b, dx, ddt, dD,
...
def _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None,
    ...
    dA_cumsum, dA_rev_cumsum, dt = _chunk_cumsum_fwd(dt, A, chunk_size, dt_bias=dt_bias, dt_softplus=dt_softplus,
                                                     dt_limit=dt_limit)
    states_right, states_b_right = _chunk_state_fwd(B, x, dt, dA_cumsum, dA_rev_cumsum, seq_idx=seq_idx,
                                                    states_in_fp32=True)
    (states, states_b), (final_states, final_states_b) = _state_passing_fwd(
        rearrange(states_right, "... p n -> ... (p n)"),
        rearrange(states_b_right, "... p n -> ... (p n)"),
        dA_cumsum[:, :, :, -1], dA_rev_cumsum[:, :, :, 0],
        initial_states=rearrange(initial_states, "... p n -> ... (p n)") if initial_states is not None else None,
        seq_idx=seq_idx, chunk_size=chunk_size, out_dtype=C.dtype)
    states, states_b, final_states, final_states_b = [rearrange(t, "... (p n) -> ... p n", n=dstate)
                                                      for t in [states, states_b, final_states, final_states_b]]
    CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    out, out_x = _chunk_scan_fwd(CB, x, dt, dA_cumsum, dA_rev_cumsum, C, states,
                                 states_b, D=D, z=z, seq_idx=seq_idx)
    return (out, out_x, dt, dA_cumsum, dA_rev_cumsum, states,
            final_states, states_b, final_states_b)

def _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, chunk_size, D=None, z=None,
                                   dt_bias=None, initial_states=None, dfinal_states=None, dfinal_states_b=None,
                                   seq_idx=None, dt_softplus=False, dt_limit=(0.0, float("inf")),
    ...
    dt_in = dt.clone()
    dA_cumsum, dA_rev_cumsum, dt = _chunk_cumsum_fwd(dt_in, A, chunk_size, dt_bias=dt_bias, dt_softplus=dt_softplus,
                                                     dt_limit=dt_limit)

    CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    states_right, states_b_right = _chunk_state_fwd(B, x, dt, dA_cumsum, dA_rev_cumsum, seq_idx=seq_idx,
                                                    states_in_fp32=True)
    (states, states_b), _ = _state_passing_fwd(
        rearrange(states_right, "... p n -> ... (p n)"),
        rearrange(states_b_right, "... p n -> ... (p n)"),
        dA_cumsum[:, :, :, -1], dA_rev_cumsum[:, :, :, 0],
        initial_states=rearrange(initial_states, "... p n -> ... (p n)") if initial_states is not None else None,
        seq_idx=seq_idx, chunk_size=chunk_size)
    states, states_b = [rearrange(t, "... (p n) -> ... p n", n=dstate) for t in [states, states_b]]
    if z is not None:
        raise NotImplementedError("z is not supported in the backward pass")
        ...
        dz = None
        outz = out
    dstates, dstates_b = _chunk_scan_bwd_dstates(C, dA_cumsum, dA_rev_cumsum, dout, seq_idx=seq_idx, dtype=states.dtype)
    ...
    dstates, dstates_b, ddA_chunk_cumsum, ddA_chunk_rev_cumsum, dinitial_states, states, states_b = _state_passing_bwd(
        ...
        dfinal_states_b=rearrange(dfinal_states_b, "... p n -> ... (p n)") if dfinal_states_b is not None else None,
        seq_idx=seq_idx,
    ...
    # The reverse is true for dstates_b and states_b.
    ...
    dx, ddt, dD_from_x = _chunk_scan_chunk_state_bwd_dx(x, dt, dA_cumsum, dA_rev_cumsum, B, CB, dout, dstates,
                                                        dstates_b,
                                                        D=D, seq_idx=seq_idx, dx=dx)
    # dB = _chunk_state_bwd_db(x, dt, dA_cumsum, dstates, seq_idx=seq_idx, ngroups=ngroups)
    dB, ddA_next, ddA_next_rev = _chunk_state_bwd_db(x, dt, dA_cumsum, dA_rev_cumsum, dstates, dstates_b,
                                                     seq_idx=seq_idx, B=B, ngroups=ngroups)
    # dB, ddA_next = _chunk_state_bwd_db_old(x, dt, dA_cumsum, dstates, seq_idx=seq_idx, B=B, ngroups=ngroups)

    # dC = _chunk_scan_bwd_dC(states[:, :-1].to(x.dtype), dA_cumsum, dout, seq_idx=seq_idx, ngroups=ngroups)
    dC, ddA_cumsum_prev, ddA_rev_cumsum_prev = _chunk_scan_bwd_dC(states.to(x.dtype), states_b.to(x.dtype), dA_cumsum,
                                                                  dA_rev_cumsum,
                                                                  dout, seq_idx=seq_idx, C=C, ngroups=ngroups)
    # Computing ddA with the dcb kernel is much slower, so we're not using it for now
    dCB = _chunk_scan_bwd_dcb(x, dt, dA_cumsum, dA_rev_cumsum, dout, seq_idx=seq_idx, ngroups=ngroups)
    ...
    ddA_rev_cumsum_prev[..., 0] += ddA_chunk_rev_cumsum
    ddA_prev = ddA_cumsum_prev.flip([-1]).cumsum(dim=-1).flip([-1]) + ddA_rev_cumsum_prev.cumsum(dim=-1)
    # ddA_prev = rev_cumsum(ddA_cumsum_prev) + ddA_rev_cumsum_prev.cumsum(dim=-1)
    ...
    ddA = _chunk_scan_bwd_ddAcs_stable(x, dt, dA_cumsum, dA_rev_cumsum, dout, CB)
    ddA += ddA_next + ddA_next_rev + ddA_prev
...
class BiMambaChunkScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None,
        ...
        (out, out_x, dt_out, dA_cumsum, dA_rev_cumsum,
         states, final_states, states_b, final_states_b) = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=D, z=z,
                                                                                          dt_bias=dt_bias, initial_states=initial_states,
                                                                                          seq_idx=seq_idx, dt_softplus=dt_softplus,
                                                                                          dt_limit=dt_limit)
        ...
        return out if not return_final_states else (out, final_states, final_states_b)
    ...
    def backward(ctx, dout, *args):
        ...
        dfinal_states_b = args[1] if ctx.return_final_states else None
        dx, ddt, dA, dB, dC, dD, dz, ddt_bias, dinitial_states = _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C,
                                                                                                ...
                                                                                                dfinal_states_b=dfinal_states_b,
                                                                                                seq_idx=seq_idx,
...
def mamba_chunk_scan_combined_bi(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None,
                                 seq_idx=None, dt_softplus=False, dt_limit=(0.0, float("inf")),
    ...
    Return:
        out: (batch, seqlen, nheads, headdim)
    """
    return BiMambaChunkScanCombinedFn.apply(x, dt, A, B, C, chunk_size, D, z, dt_bias, initial_states, seq_idx,
...
class BiMambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        if rmsnorm_weight is None:
            (out, out_x, dt_out, dA_cumsum, dA_rev_cumsum, states, final_states,
             states_b, final_states_b) = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size=chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=dt_limit)
            out = rearrange(out, "b s h p -> b s (h p)")
            rstd = None
            if d_nonssm > 0:
                out = torch.cat([_swiglu_fwd(zx0), out], dim=-1)
        else:
            out_x, _, dt_out, dA_cumsum, da_rev_cumsum, states, final_states, states_b, final_states_b = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size=chunk_size, D=D, z=None, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=dt_limit)
...
def mamba_split_conv1d_scan_combined_bi(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu", rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
    ...
    Return:
        out: (batch, seqlen, dim)
    """
    return BiMambaSplitConv1dScanCombinedFn.apply(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states, seq_idx, dt_limit, return_final_states, activation, rmsnorm_weight, rmsnorm_eps, outproj_weight, outproj_bias, headdim, ngroups, norm_before_gate)
