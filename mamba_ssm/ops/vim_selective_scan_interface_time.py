...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        start_conv = torch.cuda.Event(enable_timing=True)
        end_conv = torch.cuda.Event(enable_timing=True)
        start_linear = torch.cuda.Event(enable_timing=True)
        end_linear = torch.cuda.Event(enable_timing=True)
        start_rearrange = torch.cuda.Event(enable_timing=True)
        end_rearrange = torch.cuda.Event(enable_timing=True)
        start_scan = torch.cuda.Event(enable_timing=True)
        end_scan = torch.cuda.Event(enable_timing=True)
        start_ctx = torch.cuda.Event(enable_timing=True)
        end_ctx = torch.cuda.Event(enable_timing=True)
        ...
        # Rearrangement for delta
        start_rearrange.record()
        ...
        # Collect timings
        times = {
            'mamba_init': start_init.elapsed_time(end_init),
            'convolution': start_conv.elapsed_time(end_conv),
            'linear_projection': start_linear.elapsed_time(end_linear),
            'rearrangement': start_rearrange.elapsed_time(end_rearrange),
            'scan_operation': start_scan.elapsed_time(end_scan),
            'writing_context': start_ctx.elapsed_time(end_ctx)
        }
        return out_z, times

    @staticmethod
    @custom_bwd
    def backward(ctx, dout, times):
